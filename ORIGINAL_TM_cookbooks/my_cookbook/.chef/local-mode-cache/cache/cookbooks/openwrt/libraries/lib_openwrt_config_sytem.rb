    ...
    class System
      class System_Block
        ...
        def parse(line)
          ...
          if result
            instance_variable_set('@' + result[1], result[2])
          end
        end
      ...
      class Timeserver_Block
        ...
        def to_string
          buffer = ''
          buffer << format("config timeserver '%s'\n", @name)
          @lines.each do |line|
            buffer << format("%s\n", line)
          end
          buffer
    ...
    class Network
      class Interface_Block
        ...
        def parse(line)
          ...
          if result
            key = format('%s', result[1])
            value = format('%s', result[2])
            option[key] = value
          end
        end
        ...
        def to_string
          buffer = ''
          buffer << format("config interface '%s'\n", name)
      ...
      class Globals_Block
        ...
        def to_string
          buffer = ''
          buffer << format("config globals '%s'\n", 'globals')
          option.each do |key, value|
            buffer << format("\t option %s '%s'\n", key, value)
          end
          buffer
      ...
      class Route_Block
        ...
        def matches(target: nil, interface: nil)
          found = true
          found &= (option['target'] == target)
          found & (option['interface'] == interface)
        end
      ...
      def to_string
        buffer = ''
        buffer << global.to_string()
        buffer << "\n"
        interfaces.each do |_key, value|
